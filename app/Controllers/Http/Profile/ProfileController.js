...
class ProfileController {
	create({ view, session }) {
		return view.render('shop.profile.profileForm');
	}

	async edit({ view, session, auth, locale }) {
		const user = await auth.getUser();
		const profile = await user.profile().fetch();
		return view.render('shop.profile.profileForm', { profile });
	}
	...
	async store({ request, session, auth, response, antl }) {
		...
		// Store or Create Profile depending on 'exist profile'
		if (!profile) {
			profile = new Profile();
		}
		// Save data in Profile record
		try {
			profile.merge(profileData);
			await profile.save();
		} catch (e) {
			console.log('there was an error on saveing the profile record');
			console.log(e);
		} finally {
			session.flash({
				notification: {
