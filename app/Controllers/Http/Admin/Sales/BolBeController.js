...
const Order = use('App/Models/SalesOrderBolBe');
const OrderItem = use('App/Models/SalesOrderBolBeRow');
const Env = use('Env');
const BolApi = use('App/ZawaClasses/BolApi.js');
const parseString = require('xml2js').parseString;

class BolBeController {
	async openOrders({ view }) {
		const bolApiBe = new BolApi(Env.get('BOL_BE_PUBLIC_KEY'), Env.get('BOL_BE_PRIVATE_KEY'));
		var data = await bolApiBe.getOrders();
		...
		for (let counter1 = 0; counter1 < ordersLength; counter1++) {
			...
			order.id_order_bol_be = bolOrder.OrderId;
			...
			console.log('ORDERNR : ' + order.id_order_bol_be);
			...
			for (let counter2 = 0; counter2 < bolOrderItemsLength; counter2++) {
				...
				for (let counter3 = 0; counter3 < bolOrderItemLength; counter3++) {
					...
					orderItem.id_sales_order_bol_be = order.id;
					orderItem.id_order_bol_be = order.id_order_bol_be;
					...
					orderItem.row_total_sp_in_vat = Number(bolOrderItem.Quantity) * Number(orderItem.product_sp_in_vat);
					console.log(bolOrderItem.quantity);
					orderItem.transaction_fee = bolOrderItem.TransactionFee;
					orderItem.calc_cost_bol_be = product.total_cost_ex_vat_bol_be;
					orderItem.latest_delivery_date = bolOrderItem.LatestDeliveryDate;
					orderItem.shipping_cost_ex_vat_bol_be = product.shipping_cost_ex_vat_bol_be;
		...
		return view.render('admin.sales.orderBolBeList', { orders });
	}
}

module.exports = BolBeController;
